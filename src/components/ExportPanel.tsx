  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-xs border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="bg-slate-900 dark:bg-slate-950 text-white p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
          ...
          <div className="flex gap-2">
            <button onClick={handleExport} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 text-sm font-bold rounded-lg flex items-center gap-2 transition">
              <FileSpreadsheet className="w-4 h-4" /> Exportar CSV
            </button>
            <button onClick={handleRestoreDemo} className="bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 px-4 py-2.5 text-sm font-bold rounded-lg flex items-center gap-2 transition">
        ...
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 space-y-4">
          <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="inline-flex rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 p-0.5 text-xs">
                {['TODOS', 'INGRESO', 'SALIDA'].map(a => (
                  <button key={a} onClick={() => setFilters({ filterAccion: a as any })} className={`px-3 py-1 font-semibold rounded-md ${filterAccion === a ? 'bg-slate-800 text-white' : 'text-slate-600 dark:text-slate-300'}`}>{a}</button>
                ))}
              </div>
              <div className="inline-flex rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 p-0.5 text-xs">
                {['TODOS', 'Visita', 'Proveedor', 'Residente', 'Contratista'].map(t => (
                  <button key={t} onClick={() => setFilters({ filterTipo: t as any })} className={`px-2.5 py-1 font-semibold rounded-md ${filterTipo === t ? 'bg-blue-600 text-white' : 'text-slate-600 dark:text-slate-300'}`}>{t}</button>
                ))}
              </div>
            </div>
            <button onClick={() => { setFilters({ filterAccion: 'TODOS', filterTipo: 'TODOS', searchQuery: '' }); triggerToast('🧙 Filtros restablecidos.'); }} className="text-xs text-blue-600 dark:text-blue-400 font-bold flex items-center gap-1">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-white dark:bg-slate-800 max-h-96 overflow-y-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-900 font-bold text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                  <th className="py-2.5 px-4">Fecha / Hora</th><th className="py-2.5 px-4">RUT</th><th className="py-2.5 px-4">Nombre</th><th className="py-2.5 px-4">Clasificación</th><th className="py-2.5 px-4">Patente</th><th className="py-2.5 px-4">Acción</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700 font-medium text-slate-700 dark:text-slate-300">
                {filteredRegistros.slice(0, 100).map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    <td className="py-2 px-4">{row.timestamp}</td><td className="py-2 px-4 font-bold">{row.rut}</td><td className="py-2 px-4">{row.nombre}</td>
                    <td className="py-2 px-4"><span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-700">{row.tipoIngreso}</span></td>
                    <td className="py-2 px-4">{row.patente}</td><td className="py-2 px-4">{row.accion}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="bg-rose-50 dark:bg-rose-950/30 rounded-2xl p-5 border border-rose-200 dark:border-rose-900 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {!showClearConfirm ? (
          <button onClick={() => setShowClearConfirm(true)} className="bg-rose-600 hover:bg-rose-700 text-white px-4 py-2 text-sm font-bold rounded-xl transition">Vaciar Todo</button>
        ) : (
          <div className="flex gap-2 shrink-0">
            <button onClick={handleWipeData} className="bg-rose-700 hover:bg-rose-800 text-white px-3.5 py-2 text-xs font-bold rounded-lg">Confirmar</button>
            <button onClick={() => setShowClearConfirm(false)} className="bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-600 px-3.5 py-2 text-xs font-bold rounded-lg">Mantener</button>
          </div>
        )}
      </div>
    </div>
